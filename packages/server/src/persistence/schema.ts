...
export const AUTHORITY_SCHEMA_VERSION = 29 as const;
...
const SCHEMA_FINGERPRINTS = {
  ...
  29: "d6d074a02434bacac2df136b747d933226aca19a642b0487f5fdb1a16f725c6b",
} as const;
...
const V29_STATEMENTS = [
  `CREATE TABLE privacy_retention_attempts (
    category TEXT NOT NULL CHECK (category IN (
      'context_snapshot_payload', 'tool_sealed_side_effect_payload'
    )),
    candidate_id TEXT NOT NULL CHECK (length(candidate_id) BETWEEN 1 AND 256),
    status TEXT NOT NULL CHECK (status IN ('pending', 'dead_letter')),
    attempts INTEGER NOT NULL CHECK (attempts BETWEEN 1 AND 8),
    available_at TEXT NOT NULL,
    last_error TEXT NOT NULL CHECK (last_error = 'purge_failed'),
    updated_at TEXT NOT NULL,
    dead_lettered_at TEXT,
    PRIMARY KEY (category, candidate_id),
    CHECK (
      (status = 'pending' AND attempts < 8 AND dead_lettered_at IS NULL)
      OR
      (status = 'dead_letter' AND attempts = 8 AND dead_lettered_at IS NOT NULL)
    )
  ) STRICT`,
  `CREATE INDEX privacy_retention_attempts_pending_v29
   ON privacy_retention_attempts(status, available_at, category, candidate_id)`,
  `CREATE INDEX privacy_retention_attempts_dead_letter_v29
   ON privacy_retention_attempts(status, dead_lettered_at, category, candidate_id)`,
  `CREATE TRIGGER privacy_retention_attempts_transition_v29
   BEFORE UPDATE ON privacy_retention_attempts
   WHEN NEW.category <> OLD.category OR NEW.candidate_id <> OLD.candidate_id
     OR OLD.status <> 'pending'
     OR NEW.status NOT IN ('pending', 'dead_letter')
     OR NEW.attempts <> OLD.attempts + 1
     OR NEW.available_at <= OLD.available_at
     OR NEW.last_error <> 'purge_failed'
     OR NEW.updated_at < OLD.updated_at
     OR (NEW.status = 'pending' AND (
       NEW.attempts >= 8 OR NEW.dead_lettered_at IS NOT NULL
     ))
     OR (NEW.status = 'dead_letter' AND (
       NEW.attempts <> 8 OR NEW.dead_lettered_at IS NULL
     ))
   BEGIN SELECT RAISE(ABORT, 'privacy retention retry transition is invalid'); END`,
] as const;
...
export const AUTHORITY_V29_STATEMENT_COUNT_FOR_TEST = V29_STATEMENTS.length;
export const AUTHORITY_V29_ROLLBACK_ASSERTION_COUNT_FOR_TEST = V29_STATEMENTS.length;
export const AUTHORITY_V29_MIGRATION_CHECKSUM_FOR_TEST = migrationChecksum(
  29, "privacy-operations-retention-recovery", V29_STATEMENTS,
);
...
const MIGRATIONS = [
  ...
  defineMigration(29, "privacy-operations-retention-recovery", V29_STATEMENTS),
...
const V29_SCHEMA_CONTRACT = {
  ...V28_SCHEMA_CONTRACT,
  privacy_retention_attempts: [
    "category", "candidate_id", "status", "attempts", "available_at",
    "last_error", "updated_at", "dead_lettered_at",
  ],
} as const satisfies Readonly<Record<string, readonly string[]>>;
...
function validateAuthorityData(database: DatabaseSync, schemaVersion: number): void {
  ...
  if (schemaVersion >= 29) {
    requireNoRows(
      database,
      `SELECT 1 FROM privacy_retention_attempts
       WHERE attempts < 1 OR attempts > 8
          OR last_error <> 'purge_failed'
          OR (status = 'pending' AND (attempts >= 8 OR dead_lettered_at IS NOT NULL))
          OR (status = 'dead_letter' AND (attempts <> 8 OR dead_lettered_at IS NULL))
       LIMIT 1`,
      "privacy retention retries must retain one bounded terminal lifecycle",
    );
  }
}

...
schema v27 把四个待投递家族统一成显式生命周期：中央 `outbox_deliveries`、Deployment Profile 流、Project event shadow 以及 room cache invalidation intent。生产消费者每批最多 100 条；失败采用 250ms 起始、30s 封顶的指数 full-jitter backoff，最多尝试 8 次，随后进入持久 `dead_letter`。同一 transition 可重放但不能倒退；中央投递终态会在同一事务内结算 Project shadow。60 秒与 5 分钟 backlog 分别发 closed warning/critical metadata，不能包含 token、payload、数据库路径或任意正文。正常关闭停止取新工作并等待有界 drain；超时必须显式失败，不能静默遗失。
